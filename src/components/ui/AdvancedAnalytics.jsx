...
const AdvancedAnalytics = ({ data, title, onSliceClick, selectedSection }) => {
  ...
  if (total === 0)
    return (
      <div
        className={`${THEME.card} glass-card p-6 rounded-xl text-center text-xs ${THEME.subText}`}
      >
        No Data for {title}
      </div>
    );
  ...
  return (
    <div className={`${THEME.card} glass-card p-5 rounded-2xl mb-6`}>
      <h3
        ...
        <div className="relative w-24 h-24 flex items-center justify-center">
          <svg className="w-full h-full transform -rotate-90">
            ...
            {data.map((d, i) => {
              ...
              const isSelected = selectedSection === d.name;
